    ...
    public function index(Request $request){ 
        ...
        $data = DB::table('school_registration as a')
            ...
            ->when(!empty($date), function ($query) use ($date) {
                $query->whereDate('a.updated_at', $date);
            })
            ->orderBy('a.id', 'DESC')
            ->groupBy('a.id')
            ->paginate(10)
            ->appends(['date' => $date]); // preserve filter in pagination
  ...
  public function index2(Request $request){ 
    try {
       ...
       if (!empty($request->date)){
             $data->whereDate('a.created_at', $request->date);
        }
        ...
        return view('admin.manage_school.full', compact('data'));
  ...
  public function view_invoice($id){ 
    ...
    public function student(Request $request){
      ...
      // Load class and subject lists for dropdowns
          ...
          return view('admin.studenlist', compact('students', 'sr','total_student','classes','discounts','schools','subjects'));
   }
