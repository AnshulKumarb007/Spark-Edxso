    ...
    public function store(Request $request)
    {
        ...
        if(isset($request->registration_id_from_admin)){
            ...
            $up_school->designation        = $request->designation; 
                     ...
                     $exist = Headofschool::where('email',$request->email)->first();   
                    if($exist) {
                        return redirect()->back()->with('error', "Entered email id is already registered.");
                    }     
                    $existmb = Headofschool::where('mobile',$request->mobile)->first();  
                    if($existmb) {
                        return redirect()->back()->with('error', "Entered mobile no. is already registered.");
                    }
                // Create new record
                $up_school = new Headofschool([
                    'registration_id' => $request->registration_id_from_admin,
                ...
                $up_school->save(); 
            return redirect()->back()->with('success', "Head of school details updated successfully");
        }else{
            ...
            return redirect()->route('spark.cordinator')->with('success', "Head of school details saved successfully.");
        }
    }
