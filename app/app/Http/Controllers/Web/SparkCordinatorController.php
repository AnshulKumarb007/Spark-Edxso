...
class SparkCordinatorController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(isset($request->registration_id_from_admin)){
            ...
            $up_school->designation       = $request->designation; 
            $up_school->save();
            return redirect()->back()->with('success', "Spark coordinator details updated successfully");
        }else{
            $data = [
            ...
            return redirect(route('school.create.date'))->with('success', 'Spark coordinator details saved successfully');
        }
    }
