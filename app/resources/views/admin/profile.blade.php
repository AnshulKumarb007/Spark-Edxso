									...
									<div class="row align-items-center">
										<div class="col-md-12">
											<div class="page-header-title">
												<h5 class="text-capitalize">School Profile of  - {{$sd->school_name}} (Reg. ID   {{$user->school_id}} )</h5>
											</div>
											<ul class="breadcrumb">
...
{{-- 
                                ...
                                <div class="col-md-12 col-xl-12">
                                    <div class="card card-social">										 
                                        ...
                                        <div class="col-md-8"> 
                                            ...
                                            <b class="">Note: File should not exceed 2 MB. Accepted dimensions: 512x512 pixels. Supported formats: .jpeg, .png, .jpg only.</b>
                                            </div>
    ...
    <div class="mb-3">
       ...
       @foreach($title as $t)
            <option value="{{ $t->id }}" {{ $t->id == ($hs->title_id ?? '') ? 'selected' : '' }}>
                {{ $t->title }}
            </option>
        @endforeach
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" name="second_name" id="second_name" value="{{ $hs->second_name??'' }}" placeholder="Enter Middle Name" >
        @error('second_name')
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" name="last_name" id="third" value="{{ $hs->last_name??'' }}" placeholder="Enter Last Name" >
    ...
    <div class="mb-3">
        ...
        <select name="designation" class="form-control" aria-label="Select Designation" require id="designation-dropdown">
          ...
          @foreach($designation as $d)
            <option value="{{ $d->id }}" {{ $d->id == ($hs->designation ?? '') ? 'selected' : '' }}>
                {{ $d->designation }}
            </option>
        @endforeach
    ...
    <div class="mb-3">
        <label for="second_name" class="form-label">Middle Name</label>
        <input type="text" class="form-control" name="second_name" id="second_name" value="{{ $sp->second_name }}" placeholder="Enter Middle Name" >
        @error('second_name')
            <div class="text-danger">{{ $message }}</div>
        @enderror
    </div>
    ...
    <div class="mb-3">
        <label for="last_name" class="form-label">Last Name</label>
        <input type="text" class="form-control" name="last_name" id="third" value="{{ $sp->last_name }}" placeholder="Enter Last Name" >
    ...
    <button type="submit" class="btn btn-primary">Update</button>
            ...
            <div id="collapseThree1" class="card-body collapse" aria-labelledby="headingThree" data-parent="#accordionExample">
                                        ...
                                        <form action="{{ route('school_enroll.save') }}" method="POST">
                                            ...
                                            <input type="number" class="form-control" id="total_com_labs" name="total_com_labs" value="{{ $count }}" required min="1" max="25" placeholder="Enter number of computer labs" maxlength="2" pattern="^(?:[1-9]|1[0-9]|2[0-5])$" oninput="this.value = this.value.replace(/[^0-9]/g, ''); if (this.value.length > 1 && this.value.startsWith('0')) this.value = this.value.replace(/^0+/, ''); if (parseInt(this.value) > 25) this.value = '25'; ">
                                        ...
                                        <form action="{{ route('school_enroll.savelabs') }}" method="post">
                                            @csrf
                                            <input type="hidden" name="registration_id_from_admin" value="{{$user->id}}">
                                                    ...
                                                    <tbody id="labs-table-body">
                                                        ...
                                                        @foreach($ld as $l)
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td><input type="number" class="form-control" name="computer_qty[]" value="{{ $l->computer_qty }}" min="1" max="200" required oninput="this.value = this.value.replace(/[^0-9]/g, ''); if (this.value.length > 1 && this.value.startsWith('0')) this.value = this.value.replace(/^0+/, ''); if (parseInt(this.value) > 200) this.value = '200'; "></td>   
                                                                <td>
                                                                    <a href="{{ url('delete-profile', $l->id) }}" class="btn btn-danger">Remove</a>                                                                   
                                                                </td>  
                                                            </tr>
                                                        </form>                                                       
                                                        @endforeach
                                                    </tbody>
                                                    <tbody id="dynamic-lab-rows"></tbody>
                                                    <tbody>
                                                        <tr>
                                                            <td colspan="4">
                                                                <button type="submit" class="btn btn-primary">Update Lab Info</button>
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </table>
	...
	<template id="lab-row-template">
        <tr>
            ...
            <td><input type="text" name="labs_name[]" class="form-control" placeholder="Lab Name" readonly></td>
            <td><input type="number" name="computer_qty[]" class="form-control" placeholder="Number of Computers" min="1" max="200" required oninput="this.value = this.value.replace(/[^0-9]/g, ''); if (this.value.length > 1 && this.value.startsWith('0')) this.value = this.value.replace(/^0+/, ''); if (parseInt(this.value) > 200) this.value = '200'; "></td>
            <td></td>
        </tr>
    </template>
...
<script>
    $(document).ready(function () {
        $('#total_com_labs').on('input', function () {
            const count = parseInt($(this).val());
            const $tbody = $('#dynamic-lab-rows');
            const template = $('#lab-row-template').html();
            $tbody.empty(); 
            if (!isNaN(count) && count > 0) {
                for (let i = 0; i < count; i++) {
                    const row = $(template);
                    row.find('.sr-number').text($('#labs-table-body tr').length + i + 1);
                    row.find('input[name="labs_name[]"]').val('Lab' + (i + 1));
                    $tbody.append(row);
                }
            }
        });
        $(document).on('click', '.remove-lab', function () {
            ...
            $('#dynamic-lab-rows tr').each(function (index) {
                $(this).find('.sr-number').text($('#labs-table-body tr').length + index + 1);
            });
        });
    });
</script>
