                            ...
                            <div class="row align-items-center">
                                <div class="col-md-12">
                                    ...
                                    <ul class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item">Registered Student Lists</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        ...
                        <form method="GET" action="{{ url()->current() }}" class="mb-4">
                                <div class="row">
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <select name="vschool_id" id="exam-slot" class="form-control" required>                      
                                ...
                                <table class="table datatable" id="schoolTable">
                                    ...
                                    @endphp
                                            ...
                                            <tr>
                                                ...
                                                <td>@if(!empty($st->class_name)){{ $st->class_name }}@endif</td>
                                                <td>@if(!empty($st->class_name)){{ $st->section }}@endif</td>
                                                <td  class="text-capitalize">{{ $st->father_name }}</td>
                                                <td  class="text-capitalize">{{ $st->relative_middle_name }}</td>
                                                <td  class="text-capitalize">{{ $st->relative_last_name }}</td>
                                            ...
                                            @endphp
                                                ...
                                                <td>{{ $st->email }}</td>
                                                <td>{{ $st->utr }}</td>
                                                  <td>{{$st->razorpay_payment_id}} <br/><b @if($st->status=="success") class="text-success" @else class="text-danger" @endif>{{$st->status}}</b></td>
                                        ...
                                        border:1px solid {{ $st->verify_status === 1 ? 'orange' : ($st->verify_status === 2 ? 'green' : 'red') }};padding:2px;border-radius:4px;color:#fff;text-align: center;font-size: 11px;width: 50px;
                                            ">
                                            {{ $st->verify_status === 1 ? 'Pending' : ($st->verify_status === 2 ? 'Verified' : 'Rejected') }}
                                            </p>
                                            @else

                                            @if($st->fees_statuss == 0)
                                            <p style="background: #4b744b;padding:2px;border-radius:4px;color:#fff;text-align: center;font-size: 11px;">Details verified </p>
                                            @elseif($st->fees_statuss==1)
                                            <p style="background: green;padding:2px;border-radius:4px;color:#fff;text-align: center;font-size: 11px;">Fee Verified</p>
                                            @else
                                            <p style="background: red;padding:2px;border-radius:4px;color:#fff;text-align: center;font-size: 11px;">Fee Rejected</p>

                                            @endif
                                            
                                        @endif    
                                            </td>
                                                 <td>  {{ date('d-m-Y', strtotime($st->updated_at)) }}  </td>
                                          
                                            @if($st->verify_status == 2)
                                        
                                            @if($st->fees_statuss==1)
                                            <td><button class="btn btn-sm btn-primary paymen-verified"> ₹  <i class="fa fa-check text-white" title="Fee Verified Successfully"></button></i>
                                            </td> 
                                            @else
                                            <td><a href="" class="verify-btn btn btn-sm btn-primary" 
                                                data-student-id="{{ $st->id }}" title="Click to verify the Fee"> ₹ <i class="fa fa-check text-white"></i></a>
                                            </td>
                                            @endif
                                            @else
                                                    @if($st->fees_statuss==1)
                                                        <td><button class="btn btn-sm btn-primary"> ₹ <i class="fa fa-check text-white" style="cursor: pointer;" title="Fee Verified"></i></button></td>
                                                        @else
                                                        <td><button class="btn btn-sm btn-primary not-verify"> ₹ <i class="fa fa-check text-white  " style="cursor: pointer;" title="Please Verify Student Details First"></i></button></td>
                                                        @endif
                                            @endif
