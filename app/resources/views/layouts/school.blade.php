...
@php $user = Auth::guard('school')->user(); @endphp
	<!-- [ Pre-loader ] start -->
	<!-- <div class="loader-bg">
...
{{-- 
					...
					<li class="nav-item">
						<a href="{{route('student.result.report')}}" class="nav-link"><span class="pcoded-micon"><i class="fas fa-file"></i></span><span class="pcoded-mtext">Reports And Certificate </span></a>
					</li>
					...
					<li class="nav-item">
						...
						class="nav-link download-certificates"
						data-url="{{ route('bulk.image.download',['filename'=>$user->school_id]) }}">
					...
					<li class="nav-item">
						<a href="javascript:void(0)"
						class="nav-link download-school-certificates"
						data-url="{{ route('print-pdf',['schoolid'=>$user->school_id]) }}">
							<span class="pcoded-micon">
								<i class="fas fa-download"></i>
							</span>
							<span class="pcoded-mtext">
	...
	<header class="navbar pcoded-header navbar-expand-lg navbar-light headerpos-fixed">
		...
		<a class="mobile-menu" id="mobile-header" href="#!">
			<i class="feather icon-more-horizontal"></i>
		       ...
		       @php  $userWithRelations = \App\Models\SchoolDetails::where('registration_id',Auth::guard('school')->id())->first(); @endphp	
			...
			<ul class="navbar-nav mr-auto">
					<li class="nav-item">
						<div class="main-search open">							 
							 <img src="{{asset($user->image??'default-logo.png')}}" class="img-radius school-logo" style="border-radius:9px;">
					...
					<div class="dropdown">
						...
						<div class="dropdown-menu dropdown-menu-right notification">
							...
							<ul class="noti-body">
								  ...
								  @php 
								  $student_notificaiton =\App\Models\Student::select('students.id','full_name','middle_name','last_name','students.created_at','class.name as class_name','notiifcation_view')
								  ->join('class', 'students.class', '=', 'class.id') 
								  ->where('school_id',Auth::guard('school')->user()->school_id
								  )->orderby('students.id','desc')->get();
								  @endphp
								 
								 @foreach($student_notificaiton as $studentt_notificaiton)
								<li style="background-color: {{ $studentt_notificaiton->notiifcation_view == 0 ? '#f0f0f0' : '#ffffff' }};">
										<a href="{{ route('view.notification', ['id' => $studentt_notificaiton->id]) }}" style="padding: 0px; margin: 0px;">
											<div class="media">
												<div class="media-body">
													<p class="mt-3">
														<strong>
															{{ $studentt_notificaiton->full_name . ' ' . $studentt_notificaiton->middle_name . ' ' . $studentt_notificaiton->last_name }}
															registered in the {{ $studentt_notificaiton->class_name }} for 5 olympiads
														</strong>
														<span class="n-time text-muted">
															<i class="icon feather icon-clock m-r-10"></i>
															{{ $studentt_notificaiton->created_at->diffForHumans() }}
														</span>
													</p>
												</div>
											</div>
							...
							<div class="noti-footer">
								<a href="{{route('student.verification')}}">show all</a>
							</div>
						</div>
					</div>
				</li>
				 ...
				 <li>
					<div class="dropdown drp-user">
						...
						<div class="dropdown-menu dropdown-menu-right profile-notification">
							  ...
							  <ul class="pro-body">
								<li style="background:gray;font-size:16px;"><b class="dropdown-item">  School ID - {{$user->school_id}}</b></li>
								...
								<li><a href="{{route('school.share.link')}}" class="dropdown-item"><i class="far fa-clock" aria-hidden="true"></i> Change Exam Slots</a></li>
...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.confirm-download').forEach(function (el) {
...
<script>
document.addEventListener('DOMContentLoaded', function () {
    document.querySelector('.download-certificates').addEventListener('click', function () {
        let url = this.getAttribute('data-url');

        Swal.fire({
            title: 'Download Certificate',
            text: 'Are you sure you want to download certificates?',
            icon: 'warning',
...
<script>
	document.addEventListener('DOMContentLoaded', function () {
		document.querySelector('.download-school-certificates').addEventListener('click', function () {
			let url = this.getAttribute('data-url');
	
			Swal.fire({
				title: 'Download Certificate',
				text: 'Are you sure you want to download school certificates?',
				icon: 'warning',
				showCancelButton: true,
				confirmButtonColor: '#3085d6',
				cancelButtonColor: '#d33',
