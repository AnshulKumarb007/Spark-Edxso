                        ...
                        <form id="enrollForm" method="POST">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label for="total_enrollment" class="form-label">School's Total Strength<span class="text-danger">*</span> 
                                        <i class="fa fa-info text-black ms-1 information"
                                            data-bs-toggle="tooltip"
                                            data-bs-placement="right"
                                            title="Enter school's total strength">
                                        </i>
                                    </label>
                                    <input type="text" class="form-control" id="total_enrollment" name="total_enrollment" @if(!empty($school_enrolment)) value="{{ $school_enrolment->total_enrollment }}" @endif required placeholder="Enter school's total strength" maxlength="10" pattern="^[1-9][0-9]{0,9}$" oninput="this.value = this.value.replace(/[^0-9]/g, ''); if(this.value.startsWith('0')) this.value = this.value.replace(/^0+/, '');">
                                ...
                                <div class="col-md-6 mb-3">
                                    <label for="class_1_to_8_enroll" class="form-label">Student Strength - Class 1 to 8<span class="text-danger">*</span>
                                        <i class="fa fa-info text-black ms-1 information"
                                            data-bs-toggle="tooltip"
                                            data-bs-placement="right"
                                            title="Enter student strength - Class 1 to 8.">
                                        </i>
                                    </label>
                                    <input type="text" class="form-control" id="class_1_to_8_enroll" name="class_1_to_8_enroll" @if(!empty($school_enrolment)) value="{{ $school_enrolment->class_1_to_8_enroll }}" @endif required placeholder="Enter student strength - class 1 to 8" maxlength="10" pattern="^[1-9][0-9]{0,9}$" oninput="this.value = this.value.replace(/[^0-9]/g, ''); if(this.value.startsWith('0')) this.value = this.value.replace(/^0+/, '');">
                            ...
                            <div class="mb-3">
                                <label for="total_com_labs" class="form-label">Number of Computer Labs<span class="text-danger">*</span>
                                    <i class="fa fa-info text-black ms-1 information"
                                        data-bs-toggle="tooltip"
                                        data-bs-placement="right"
                                        title="Enter number of computer labs (min 1 & max 25).">
                                    </i>
                                </label>
                                <input type="text" class="form-control" id="total_com_labs" name="total_com_labs"
                                @if(!empty($school_enrolment)) value="{{ $school_enrolment->total_com_labs }}" @endif required min="1" max="25" placeholder="Enter number of computer labs" maxlength="2" pattern="^(?:[1-9]|1[0-9]|2[0-5])$" oninput="this.value = this.value.replace(/[^0-9]/g, ''); if (this.value.length > 1 && this.value.startsWith('0')) this.value = this.value.replace(/^0+/, ''); if (parseInt(this.value) > 25) this.value = '25'; ">
                            ...
                            <div class="row mb-3">
                                <div class="col-sm-4">
                                    <a href="{{ route('school.create.date') }}" class="btn btn-outline-primary w-100"><i class="fa fa-arrow-left"></i> Back</a>
                                </div>
                                <div class="col-sm-8">
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="previewModalLabel">Review Your Details</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body" id="previewContainer"></div>
                    <div class="row p-3 mb-3 float-right">
                        <div class="col-sm-2">
                            <a href="{{ route('school.create') }}" class="btn btn-outline-primary w-100"><i class="fa fa-arrow-left"></i> Back</a>
                        </div>
                        <div class="col-sm-4">
                            <a href="#" onclick="saveButton(event)" data-bs-dismiss="modal" aria-label="Close" class="btn btn-primary w-100"> Save & Proceed <i class="fa fa-arrow-right"></i></a>
                        </div>
                    </div>
                </div>
            </div>
...
@push('scripts')
    ...
    <script>
        function saveButton(event) {
            ...
            showloader();
            const form1 = document.querySelector('#enrollForm');
            ajaxSave(form1, 1, function(result) {
                if (result === true) {
                    setTimeout(() => {
        ...
        document.getElementById('enrollForm').addEventListener('submit', function(e) {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "Please confirm to submit the entered details.",
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    buttonDisable(false);
                    showloader();
                    ajaxSave(form, 1, function(result) {
                        if (result === true) {
                            setTimeout(() => {
                                $('#myForm').submit();
                            },4000);
                    ...
                    buttonDisable(false);
                    ajaxSave(form, 2, function(result) {
                        if (result === true) {
                            $.ajax({
                                url: "{{ url('preview') }}",
                                type: 'GET',
                                beforeSend: function () {
                                    showloaderwithouttext();
                                },
        ...
        function ajaxSave(form,$id,callback){
            $.ajax({
        ...
        function showloader() {
            ...
            const oldMessage = preloader.querySelector('.preloader-message');
            if (oldMessage) {
                oldMessage.remove();
            }
        ...
        function showloaderwithouttext() {
            const preloader = document.querySelector('#preloader');
            preloader.style.display = 'flex';
        }
        function hideloader(){
