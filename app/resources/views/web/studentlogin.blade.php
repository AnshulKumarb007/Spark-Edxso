...
@section('content')
 <style>
    ...
    @endif
        <form  method="Post" action="{{ url('student-store-login') }}">
            ...
            @csrf
                ...
                <div class="mb-3">
                    ...
                    <div class="left-section">
                        <h4 class="f-w-400">Student Login</h4> 
       ...
       title="Only letters, numbers, slashes (/) and hyphens (-) are allowed." />
            ...
            <div class="mb-3">
                    <div class="header-container">
                            ...
                            <label for="enrollInput" class="input-label">
                                 Your registered mobile no./email id
                                </label>
                    </div> 
                        <input type="text" id="mobile" minlength="5"  maxlength="100" name="mobile" class="form-control" placeholder="Enter mobile number or email id" required autocomplete="off" />
            ...
            <button type="submit" id="submitBtn" class="btn btn-primary w-100">Login</button>
                        ...
                        <p class="mt-4 text-center">
										Don’t have an account? <a href="{{ str_replace('/sl', '/st', url()->full()) }}" class="text-primary font-weight-bold">Register Now</a>
									</p>
...
<script>
  ...
  function validateForm() {
    const isAdmnoValid = admnoInput.checkValidity();
    const isMobileValid = mobileInput.checkValidity();

    if (isAdmnoValid && isMobileValid) {
  ...
  // Run validation on keyup for both inputs
  admnoInput.addEventListener('keyup', validateForm);
  mobileInput.addEventListener('keyup', validateForm);
</script>
...
<script>
  ...
  input.addEventListener('keyup', function () {
    ...
    if (/^\d+$/.test(value)) {
      // Input is only digits (mobile number)
      if (value.length >= 9) {
        input.setAttribute('maxlength', '10');
        input.setAttribute('minlength', '10');

        // Force trim if over 10 digits
        if (value.length > 10) {
      ...
      // Input contains non-digit characters (email)
      input.setAttribute('maxlength', '100');
      input.setAttribute('minlength', '10');

      // Optional: Trim the input if it exceeds 100 characters
      if (value.length > 100) {
